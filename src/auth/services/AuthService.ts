...
const AuthService = {
  ...
  async login(credentials: LoginRequestDto): Promise<AuthResponseDto> {
    try {
      const response = await axiosInstance.post<
        WebApiResponse<AuthResponseDto>
  ...
  async recoverPassword(credentials: ForgotPassDto): Promise<void> {
    try {
      const response = await axiosInstance.post<WebApiResponse<void>>(
        "/auth/forgot",
        credentials
      );
      if (!response.data.success) {
        ...
        throw new Error("Error inesperado al enviar el correo de recuperación");
    }
  },
