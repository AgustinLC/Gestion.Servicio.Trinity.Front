...
const PERIOD_STATE_CONFIG: Record<PeriodState, { label: string; badgeClass: string }> = {
...
const Resume = () => {
    ...
    const fetchBillChart = async (idPeriod: number) => {
        ...
        try {
            const response = await getData<BillCountsDto>(`/operator/bill-counts/${idPeriod}`);
    ...
    const readingsPct = totalReadings > 0 ? Math.round((doneReadings / totalReadings) * 100) : 0;
    ...
    const pagasPct = hasInvoiceData ? Math.round((paidBills / emittedBills) * 100) : 0;
    const impagasPct = hasInvoiceData ? Math.round((unpaidBills / emittedBills) * 100) : 0;
    ...
    const selectedPeriodInfo = periods.find((period) => period.idPeriod === selectedPeriod);
    ...
    const effectiveTotalReadings = isViewingActivePeriod
        ? totalReadings
        : (emittedBills > 0 ? emittedBills : totalReadings);
    const effectiveDoneReadings = isViewingActivePeriod ? doneReadings : effectiveTotalReadings;
    const effectiveReadingsPct = isViewingActivePeriod ? readingsPct : (effectiveTotalReadings > 0 ? 100 : 0);

    // Estado general del período + paso activo del pipeline: el primer paso
    // incompleto es el activo, los anteriores quedan "done", los
    // posteriores "pending".
    const periodState: PeriodState = effectiveDoneReadings === 0 ? "notStarted" : !hasInvoiceData ? "inProgress" : "invoiced";
    const periodStateConfig = PERIOD_STATE_CONFIG[periodState];
    const pipelineStep = effectiveDoneReadings === 0 || effectiveDoneReadings < effectiveTotalReadings ? 1 : !hasInvoiceData ? 2 : 3;
    // No se puede emitir con lecturas incompletas: el botón "Emitir facturas"
    // del estado vacío se deshabilita mientras falten.
    const readingsIncomplete = effectiveTotalReadings === 0 || effectiveDoneReadings < effectiveTotalReadings;
    const stepStateClass = (step: number): string =>
        step < pipelineStep ? "rr-pipeline-step-done" : step === pipelineStep ? "rr-pipeline-step-active" : "rr-pipeline-step-pending";

    const CTA_CONFIG: Record<CtaKey, { label: string; onClick: () => void }> = {
        start: { label: "Comenzar carga de lecturas", onClick: () => navigate("/dashboard/operator/readings/take") },
        continue: { label: "Continuar carga", onClick: () => navigate("/dashboard/operator/readings/take") },
        emit: { label: "Emitir facturas", onClick: () => navigate("/dashboard/operator/bills/generate") },
        view: { label: "Ver cobranza", onClick: () => navigate("/dashboard/operator/bills/management") },
    };
    const ctaKey: CtaKey =
        effectiveDoneReadings === 0 ? "start" : effectiveDoneReadings < effectiveTotalReadings ? "continue" : !hasInvoiceData ? "emit" : "view";
    ...
    const suspendedFrac = totalUsers > 0 ? (suspendedUsers / totalUsers) * 100 : 0;
    const inactiveFrac = totalUsers > 0 ? (inactiveUsers / totalUsers) * 100 : 0;

    const missingMeters = data?.missingMeters ?? 0;
    ...
    const hasDebtAmountData = debtors.some((debtor) => (debtor.debts?.length ?? 0) > 0);
    const debtorsTotalAmount = debtors.reduce(
        (total, debtor) => total + (debtor.debts?.reduce((sum, item) => sum + item.amount, 0) ?? 0),
        0
    );
    const debtorsPctOfPadron = totalUsers > 0 ? (debtorsCount / totalUsers) * 100 : 0;
    ...
    const feeRows = useMemo(() => {
        const rows = (data?.usersForFee ?? []).slice().sort((a, b) => b.count - a.count);
        const maxCount = Math.max(1, ...rows.map((row) => row.count));
        return rows.map((row, index) => ({
            fee: row.fee,
            count: row.count,
            widthPct: (row.count / maxCount) * 100,
            pctOfTotal: totalUsers > 0 ? Math.round((row.count / totalUsers) * 100) : 0,
        ...
        <div className="content-fade-in">
            ...
            >
                <div className="rr-header-controls">
                    <div className="rr-info-chip">{data?.activeModality || "—"}</div>
                    <div className="rr-info-chip">{data?.activeUnitService || "—"}</div>
                    <CustomSelect
                ...
                {loading ? (
                    <>
                        ...
                        <div className="rr-card rr-pipeline-card">
                            <div className="skeleton skeleton-line" style={{ width: 160, height: 17 }}></div>
                            <div className="rr-pipeline-steps">
                                {[0, 1, 2].map((step) => (
                                    <div key={step} className={`rr-pipeline-step ${step === 2 ? "rr-pipeline-step-last" : ""}`}>
                                        <div className="skeleton skeleton-line" style={{ width: 90, height: 14 }}></div>
                                        <div className="skeleton" style={{ width: "100%", height: 6, borderRadius: 999 }}></div>
                                        <div className="skeleton skeleton-line" style={{ width: 110, height: 11 }}></div>
                                    </div>
                                ))}
                            </div>
                            <div className="skeleton skeleton-line" style={{ width: 190, height: 46, borderRadius: 10 }}></div>
                        </div>
                            ...
                            {[0, 1, 2].map((card) => (
                                <div key={card} className="rr-padron-card">
                                    <div className="rr-padron-head">
                                        <div className="skeleton skeleton-line" style={{ width: 110, height: 11 }}></div>
                                        <div className="skeleton" style={{ width: 26, height: 26, borderRadius: 7 }}></div>
                                    </div>
                                    <div className="skeleton skeleton-line" style={{ width: 90, height: 44 }}></div>
                        ...
                        <div className="rr-analysis-row">
                            <div className="rr-card rr-fee-card">
                                <div className="rr-fee-head">
                                    <div className="skeleton skeleton-line" style={{ width: 140, height: 16 }}></div>
                                    <div className="skeleton skeleton-line" style={{ width: 120, height: 11 }}></div>
                                </div>
                                <div className="rr-fee-rows">
                                    {[100, 40, 20, 20].map((width, index) => (
                                        <div key={index} className="rr-fee-row">
                                            <div className="skeleton skeleton-line" style={{ width: "80%", height: 12 }}></div>
                                            <div className="skeleton" style={{ width: `${width}%`, height: 10, borderRadius: 999 }}></div>
                                            <div className="skeleton skeleton-line" style={{ width: "100%", height: 12 }}></div>
                                            <div className="skeleton skeleton-line" style={{ width: "100%", height: 12 }}></div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="rr-card rr-billing-card">
                                <div className="rr-billing-head">
                                    <div className="skeleton skeleton-line" style={{ width: 100, height: 16 }}></div>
                                ...
                                <div className="rr-billing-main">
                                    <div className="rr-donut-row">
                                        <div className="skeleton rr-donut rr-donut-lg" style={{ borderRadius: "50%" }}></div>
                                        <div className="rr-billing-stats">
                                            <div className="skeleton" style={{ height: 78, borderRadius: 12 }}></div>
                                            <div className="skeleton" style={{ height: 78, borderRadius: 12 }}></div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <>
                        ...
                        <div className="rr-card rr-pipeline-card">
                            <div className="rr-pipeline-title">
                                <div className="rr-pipeline-title-text">Estado del período</div>
                                <div className={`rr-badge ${periodStateConfig.badgeClass}`}>
                            ...
                            <div className="rr-pipeline-steps">
                                <div className={`rr-pipeline-step ${stepStateClass(1)}`}>
                                    <div className="rr-pipeline-step-head">
                                        <div className="rr-pipeline-step-circle">
                                            {pipelineStep > 1 ? <i className="bi bi-check-lg"></i> : "1"}
                                        </div>
                                        <div className="rr-pipeline-step-name">Lecturas</div>
                                    </div>
                                    <div className="rr-pipeline-step-track">
                                        <div className="rr-pipeline-step-fill" style={{ width: `${effectiveReadingsPct}%`, backgroundColor: "#0F6BFF" }}></div>
                                    </div>
                                    <div className="rr-pipeline-step-caption">{effectiveDoneReadings} de {effectiveTotalReadings} registradas</div>
                                </div>

                                <div className={`rr-pipeline-step ${stepStateClass(2)}`}>
                                    <div className="rr-pipeline-step-head">
                                        <div className="rr-pipeline-step-circle">
                                            {pipelineStep > 2 ? <i className="bi bi-check-lg"></i> : "2"}
                                        </div>
                                        <div className="rr-pipeline-step-name">Facturación</div>
                                    </div>
                                    <div className="rr-pipeline-step-track">
                                        <div
                                            className="rr-pipeline-step-fill"
                                            style={{ width: hasInvoiceData ? "100%" : "0%", backgroundColor: "#0F6BFF" }}
                                        ></div>
                                    </div>
                                    <div className="rr-pipeline-step-caption">{emittedBills} facturas emitidas</div>
                                </div>

                                <div className={`rr-pipeline-step rr-pipeline-step-last ${stepStateClass(3)}`}>
                                    <div className="rr-pipeline-step-head">
                                        <div className="rr-pipeline-step-circle">3</div>
                                        <div className="rr-pipeline-step-name">Cobranza</div>
                                    </div>
                                    <div className="rr-pipeline-step-track">
                                        <div className="rr-pipeline-step-fill" style={{ width: `${pagasPct}%`, backgroundColor: "#18A957" }}></div>
                                    </div>
                                    <div className="rr-pipeline-step-caption">{paidBills} de {emittedBills} cobradas</div>
                                </div>
                            </div>

                            <div className="rr-btn rr-btn-primary rr-pipeline-cta" onClick={ctaConfig.onClick}>
                                {ctaConfig.label}
                            </div>
                        </div>
                        ...
                        <div className="rr-padron-row">
                            <div className="rr-padron-card">
                                <div className="rr-padron-head">
                                    <div className="rr-padron-eyebrow">Padrón de usuarios</div>
                                    <Link to="/dashboard/operator/users" aria-label="Ver usuarios" className="rr-padron-icon-link">
                                        <i className="bi bi-arrow-up-right"></i>
                                    </Link>
                                </div>
                                <div className="rr-padron-figure">
                                    <div className="rr-padron-value">{totalUsers}</div>
                                </div>
                                <div className="rr-stack-track">
                                    <div className="rr-stack-seg" style={{ width: `${activeFrac}%`, backgroundColor: "#18A957" }}></div>
                                    <div className="rr-stack-seg" style={{ width: `${suspendedFrac}%`, backgroundColor: "#E8890C" }}></div>
                                    <div className="rr-stack-seg" style={{ width: `${inactiveFrac}%`, backgroundColor: "#8B97A8" }}></div>
                                </div>
                                <div className="rr-legend">
                                    <div className="rr-legend-item">
                                        <div className="rr-legend-dot" style={{ backgroundColor: "#18A957" }}></div>
                                        <span><b>{activeUsers}</b> activos</span>
                                    </div>
                                    <div className="rr-legend-item">
                                        <div className="rr-legend-dot" style={{ backgroundColor: "#E8890C" }}></div>
                                        <span><b>{suspendedUsers}</b> suspendidos</span>
                                    </div>
                                    <div className="rr-legend-item">
                                        <div className="rr-legend-dot" style={{ backgroundColor: "#8B97A8" }}></div>
                                        <span><b>{inactiveUsers}</b> inactivos</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="rr-padron-card">
                                <div className="rr-padron-head">
                                    <div className="rr-padron-eyebrow">Medidores faltantes</div>
                                    <Link to="/dashboard/operator/users" aria-label="Ver medidores" className="rr-padron-icon-link">
                                        <i className="bi bi-arrow-up-right"></i>
                                    </Link>
                                </div>
                                <div className="rr-padron-figure">
                                    <div className="rr-padron-value">{missingMeters}</div>
                                    {missingMeters === 0 ? (
                                        ...
                                        <div className="rr-tile-pill rr-tile-pill-danger">
                                            <div className="rr-tile-pill-dot"></div>
                                            <span>Bloquea {missingMeters} lecturas</span>
                                        </div>
                                    )}
                                </div>
                                <div className="rr-padron-note">
                                    {missingMeters === 0
                                        ? "Todas las unidades tienen medidor asignado, ninguna bloquea la carga de lecturas."
                                        : `Estas unidades no tienen medidor asignado y no pueden cargar lecturas hasta resolverlo.`}
                                </div>
                            </div>

                            <div className="rr-padron-card">
                                <div className="rr-padron-head">
                                    <div className="rr-padron-eyebrow">Deuda acumulada</div>
                                    <Link to="/dashboard/operator/debt-disconnection" aria-label="Ver deudores" className="rr-padron-icon-link">
                                        <i className="bi bi-arrow-up-right"></i>
                                    </Link>
                                </div>
                                <div className="rr-padron-figure">
                                    <div className="rr-padron-value">{debtorsLoaded ? debtorsCount : "—"}</div>
                                    <div className="rr-padron-sub">
                                        deudores{totalUsers > 0 ? ` · ${debtorsPctOfPadron.toLocaleString("es-AR", { maximumFractionDigits: 1 })}% del padrón` : ""}
                                    </div>
                                </div>
                                <div className="rr-padron-debt-row">
                                    <div className="rr-padron-debt-track">
                                        <div className="rr-padron-debt-fill" style={{ width: `${debtorsPctOfPadron}%` }}></div>
                                    </div>
                                    {hasDebtAmountData && <div className="rr-padron-debt-amount">{formatCurrencyCompact(debtorsTotalAmount)}</div>}
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="rr-analysis-row">
                            <div className="rr-card rr-fee-card">
                                <div className="rr-fee-head">
                                    <div className="rr-card-title">Usuarios por tarifa</div>
                                    <div className="rr-card-title-sub">Distribución del padrón</div>
                                </div>
                                <div className="rr-fee-rows">
                                    {feeRows.map((row) => (
                                        <div key={row.fee} className="rr-fee-row">
                                            <div className="rr-fee-label">{row.fee}</div>
                                            <div className="rr-fee-track">
                                                <div className="rr-fee-fill" style={{ width: `${row.widthPct}%`, backgroundColor: row.color }}></div>
                                            </div>
                                            <div className="rr-fee-value">{row.count}</div>
                                            <div className="rr-fee-pct">{row.pctOfTotal}%</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div className="rr-card rr-billing-card">
                                <div className="rr-billing-head">
                                    <div className="rr-card-title">Facturación</div>
                                    <div className="rr-billing-head-right">
                                        <div className="rr-billing-period">
                                            {selectedPeriodInfo?.label ?? "—"}
                                            {selectedPeriodInfo?.active && <span className="rr-billing-period-actual"> (actual)</span>}
                                        </div>
                                        ...
                                        {hasInvoiceData && (
                                            <Link to="/dashboard/operator/bills/management" aria-label="Gestionar facturas" className="rr-padron-icon-link">
                                                <i className="bi bi-arrow-up-right"></i>
                                            </Link>
                                        )}
                                    </div>
                                ...
                                {loadingChart ? (
                                    <div className="d-flex align-items-center justify-content-center flex-grow-1 gap-3">
                                        <div className="skeleton flex-shrink-0" style={{ width: 150, height: 150, borderRadius: "50%" }}></div>
                                            ...
                                            <div className="rr-donut-row">
                                                <div className="rr-donut rr-donut-lg">
                                                    ...
                                                    <div className="rr-donut-center rr-donut-center-lg">
                                                        <div className="rr-donut-pct rr-donut-pct-lg">{pagasPct}%</div>
                                                        <div className="rr-donut-caption">cobrado</div>
                                                    </div>
                                                </div>
                                                <div className="rr-billing-stats">
                                                    <div className="rr-billing-stat rr-billing-stat-success">
                                                        <div className="rr-billing-stat-head">
                                                            <div className="rr-billing-stat-label">
                                                                <div className="rr-billing-legend-dot" style={{ backgroundColor: "#18A957" }}></div>
                                                                <span>Pagas</span>
                                                            </div>
                                                            <div className="rr-billing-stat-pct">{pagasPct}%</div>
                                                        </div>
                                                        <div className="rr-billing-stat-value">{paidBills}</div>
                                                    </div>
                                                    <div className="rr-billing-stat rr-billing-stat-danger">
                                                        <div className="rr-billing-stat-head">
                                                            <div className="rr-billing-stat-label">
                                                                <div className="rr-billing-legend-dot" style={{ backgroundColor: "#E5484D" }}></div>
                                                                <span>Impagas</span>
                                                            </div>
                                                            <div className="rr-billing-stat-pct">{impagasPct}%</div>
                                                        </div>
                                                        <div className="rr-billing-stat-value">{unpaidBills}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="rr-billing-narrow">
                                            <div className="rr-donut rr-donut-sm">
                                                ...
                                                <div className="rr-donut-center rr-donut-center-sm">
                                                    <div className="rr-donut-pct rr-donut-pct-sm">{pagasPct}%</div>
                                                    <div className="rr-donut-caption">cobrado</div>
                                                </div>
                                            </div>
                                            <div className="rr-billing-list">
                                                <div className="rr-billing-list-item">
                                                    <div className="rr-billing-legend-dot" style={{ backgroundColor: "#18A957" }}></div>
                                                    <span className="rr-billing-list-label">Pagas</span>
                                                    <span className="rr-billing-list-value">{paidBills}</span>
                                                </div>
                                                <div className="rr-billing-divider"></div>
                                                <div className="rr-billing-list-item">
                                                    <div className="rr-billing-legend-dot" style={{ backgroundColor: "#E5484D" }}></div>
                                                    <span className="rr-billing-list-label">Impagas</span>
                                                    <span className="rr-billing-list-value">{unpaidBills}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="rr-empty-billing">
                                        ...
                                        <div className="rr-empty-title">Todavía no hay facturas emitidas</div>
                                        <div className="rr-empty-body">
                                            Las estadísticas de cobranza aparecen al emitir el primer lote del período.
                                        </div>
                                        <div
                                            className={`rr-btn rr-btn-secondary align-self-start ${readingsIncomplete ? "rr-btn-disabled" : ""}`}
                                            onClick={readingsIncomplete ? undefined : () => navigate("/dashboard/operator/bills/generate")}
                                        >
                                            Emitir facturas
                                        </div>
                                    </div>
