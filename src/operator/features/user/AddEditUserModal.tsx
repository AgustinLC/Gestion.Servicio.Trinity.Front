...
import {
    useConfirmDiscard,
    onBackdropClick,
} from "../../../shared/hooks/useConfirmDiscard";

// Sobrescribimos los 3 campos para que en el formulario sean strings
type FormValues = Omit<
    UserDto,
    "digitalInvoiceAdhered" | "ivaInvoiceAdhered" | "pdfInvoiceAdhered"
> & {
...
const buildFormValues = (source?: UserDto | null): Partial<FormValues> => {
    ...
    return {
        ...source,
        digitalInvoiceAdhered:
            source.digitalInvoiceAdhered === true
                ? "true"
                : source.digitalInvoiceAdhered === false
                  ? "false"
                  : "",
        ivaInvoiceAdhered:
            source.ivaInvoiceAdhered === true
                ? "true"
                : source.ivaInvoiceAdhered === false
                  ? "false"
                  : "",
        pdfInvoiceAdhered:
            source.pdfInvoiceAdhered === true
                ? "true"
                : source.pdfInvoiceAdhered === false
                  ? "false"
                  : "",
        residenceDto: {
            ...(source.residenceDto || {}),
...
const UserForm: React.FC<UserFormProps> = ({
    ...
    const summaryFeeName = fees.find(
        (fee) => String(fee.idFee) === String(user?.residenceDto?.idFee)
    )?.name;
    const summaryInitials = `${(user?.firstName?.[0] || "").toUpperCase()}${(user?.lastName?.[0] || "").toUpperCase()}`;
    const summaryAvatarColor = getAvatarColor(
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        try {
            ...
            if (payload.digitalInvoiceAdhered !== undefined) {
                payload.digitalInvoiceAdhered =
                    payload.digitalInvoiceAdhered === "true";
            }
            if (payload.ivaInvoiceAdhered !== undefined) {
                payload.ivaInvoiceAdhered =
                    payload.ivaInvoiceAdhered === "true";
            }
            if (payload.pdfInvoiceAdhered !== undefined) {
                payload.pdfInvoiceAdhered =
                    payload.pdfInvoiceAdhered === "true";
            }
                ...
                <div className="user-summary-card mb-3">
                    <div className="d-flex align-items-center flex-wrap gap-3">
                        <div
                            ...
                            style={{
                                backgroundColor: summaryAvatarColor.bg,
                                color: summaryAvatarColor.color,
                            }}
                        >
                            {summaryInitials || "US"}
                        </div>
                        ...
                        <div className="user-summary-stat">
                            <div className="user-summary-stat-label text-muted small">
                                Fecha de registro
                            </div>
                            <span className="badge-soft badge-soft-neutral">
                                {formatDate(user.dateRegister)}
                            </span>
                        </div>
                        ...
                        {user.status && (
                            <div className="user-summary-stat">
                                ...
                                <span
                                    className={`badge-soft ${STATUS_BADGE_CLASS[user.status as Status]}`}
                                >
                                    <span
                        ...
                        <div className="user-summary-stat">
                            ...
                            <span className="badge-soft badge-soft-success">
                                {user.residenceDto?.serialNumber
                                    ? `Medidor ${user.residenceDto.serialNumber}`
                                    : "-"}
                            </span>
                        </div>
                    </div>
            ...
            <Form onSubmit={handleSubmit(onSubmit)}>
                ...
                <Row>
                    <Col md={6}>
                        <FormSectionHeader
                            ...
                            subtitle="Datos de contacto e identificación del usuario."
                        />
                    ...
                    <Col md={6}>
                        <FormSectionHeader
                            icon="bi bi-geo-alt"
                            title="Domicilio"
                            subtitle="Dirección donde se presta el servicio."
                        />
                        ...
                        <Row>
                            <Col sm={6}>
                                <Form.Group className="mb-2">
                                    ...
                                    <Form.Control.Feedback type="invalid">
                                        {
                                            errors.residenceDto?.idLocation
                                                ?.message
                                        }
                                    </Form.Control.Feedback>
                                </Form.Group>
                        ...
                        <Row>
                            <Col sm={user ? 12 : 6}>
                                <Form.Group className="mb-2">
                                    ...
                                    <Form.Control.Feedback type="invalid">
                                        {
                                            errors.residenceDto?.serialNumber
                                                ?.message
                                        }
                                    </Form.Control.Feedback>
                                </Form.Group>
                            ...
                            {!user && (
                                <Col sm={6}>
                                    <Form.Group className="mb-2">
                                        ...
                                        <Form.Control.Feedback type="invalid">
                                            {
                                                errors.residenceDto?.valueMeter
                                                    ?.message
                                            }
                                        </Form.Control.Feedback>
                                    </Form.Group>
                    ...
                    <Col md={user ? 6 : 12}>
                        <div className="form-section-panel mt-1">
                            <FormSectionHeader
                                icon="bi bi-gear"
                                title="Configuración"
                                subtitle="Tarifa y preferencias de facturación del servicio."
                            />
                ...
                <div className="form-modal-footer d-flex justify-content-between align-items-center flex-wrap gap-3">
                    <HintBox>
                        ...
                        {user
                            ? "Los cambios realizados afectarán a todas las futuras facturas y operaciones del usuario."
...
const AddEditModal: React.FC<AddEditModalProps> = ({
    ...
    const {
        ...
        cancelDiscard,
        setIsDirty,
    } = useConfirmDiscard({ onHide, alwaysConfirm: false });
