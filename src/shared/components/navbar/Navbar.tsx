            ...
            <nav ref={navRef} className="navbar navbar-expand-lg navbar-dark bg-primary fixed-top">
                <div className="container-fluid">
                    ...
                    <div className="d-flex align-items-center gap-2">
                        {isAuthenticated && (
                            <button type="button" className="navbar-icon-btn navbar-hamburger d-lg-none" onClick={toggleSidebar} title="Abrir/cerrar menú">
                        ...
                        {isAuthenticated ? (
                            <>
                                ...
                                {userRole === "ROLE_ADMIN" && (
                                    <>
                                        <li className="nav-item">
                                            <Link to="/dashboard/admin/workers" className={`nav-link navbar-link ${isActive("/dashboard/admin") ? "navbar-link-active" : ""}`}>
                                                <i className="bi bi-shield-check fs-5"></i>
                                                <span className="d-none d-lg-inline">Panel de Administración</span>
                                            </Link>
                                        </li>
                                        <li className="nav-item">
                                            <Link to="/dashboard/operator/resume" className={`nav-link navbar-link ${isActive("/dashboard/operator") ? "navbar-link-active" : ""}`}>
                                ...
                                {userRole === "ROLE_USER" && (
                                    <li className="nav-item">
                                        <Link to="/dashboard/user/resume" className={`nav-link navbar-link ${isActive("/dashboard/user") ? "navbar-link-active" : ""}`}>
                                            <i className="bi bi-diagram-3 fs-5"></i>
                                        ...
                                        <Dropdown.Menu renderOnMount>
                                            <div className="navbar-dropdown-header d-flex align-items-center gap-3 px-3 py-2">
                                                ...
                                                <div style={{ minWidth: 0 }}>
                                                    ...
                                                    {currentUser?.username && <div className="text-muted small text-truncate">{currentUser.username}</div>}
                                                </div>
                                            </div>
                                            <Dropdown.Divider />
